...
```
ui/manager_window.py        # メインの管理ウィンドウ
ui/widgets/task_tree.py     # QTreeWidget ベースのツリービュー
ui/widgets/priority_button.py  # 🔴🟡⚪ 切替ボタン
ui/widgets/deadline_picker.py  # プリセット + カレンダー
```

機能要件：
- 全プロジェクト→フェーズ→タスクをツリーで表示
- 各レベルで「追加」「編集」「削除」「並び替え（↑↓）」
- 締切・優先度・状態をその場で編集
- 編集は即座にDBに反映（Database クラスを使う）
