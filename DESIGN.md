...
| # | 機能 | 対象ファイル | 概要 |
|---|---|---|---|
| D | **カレンダー曜日色** | kanban_window.py | DeadlinePicker のカレンダーで土曜=青、日曜=赤 |
| E | **完了タスクの自動並び替え** | kanban_window.py | カンバンでタスクを完了にすると、そのフェーズの末尾へ自動で移動（`order_index` 更新） |
| F | **タスクの並び替え（フェーズ内＋フェーズ間）** | kanban_window.py | カードに ↑↓ボタン or D&D で並べ替え。フェーズ間移動も可能（`phase_id` と `order_index` を更新） |
| G | **タイマー画面の累計時間表示** | timer_window.py | 選択中タスクの「プロジェクト累計」「フェーズ累計」を表示。ボタンで表示/非表示トグル |
| H | **累計時間の表示モード切替** | timer_window.py | Gの累計を「時間」⇔「人工（=8h/日換算）」で切り替え可能に |
