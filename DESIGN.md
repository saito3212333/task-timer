...
- [x] **ステップ2**：データモデル + SQLite + CRUD（CRUD smoke test pass済）
- [x] **ステップ3**：管理画面（ツリー + 追加/編集/削除/並び替え）
- [ ] **ステップ4**：タイマー画面（プロジェクト/タスク選択 + ストップウォッチ + time_logs保存）
- [ ] **ステップ5**：テンプレート機能（YAML読込・保存） + ボトムアップ集計表示
...
- 部分更新（`update_task(id, status='done')` 形式）
- ツリーロード（Project → Phase → Task の階層取得）
- FK cascade（プロジェクト削除でフェーズ・タスクも消える）
- WALモード（クラッシュ耐性）

### テストデータ

`data/task_timer.db` に「PEARLサイト リダイレクト」プロジェクト1件・「ドメイン設定」フェーズ1件・タスク2件・タイムログ1件のテストデータ残存。
リセットしたい場合：`rm -f data/task_timer.db data/task_timer.db-wal data/task_timer.db-shm`

---

## 11. 次回セッションへの引き継ぎ

### 次に着手するのは：ステップ4（タイマー画面）

実装すべきもの：

```
ui/timer_window.py     # タイマー画面 380×480
```

機能要件：
- プロジェクト/フェーズ/タスクをドロップダウンで選択
- ストップウォッチ（▶/⏹ ボタン）
- 停止時に time_logs へ保存（started_at / ended_at / duration_sec）
- 管理画面を開くボタン（ManagerWindow を表示）

### ステップ3完了メモ（2026-05-04）

実装済み：`ui/manager_window.py`（単一ファイル構成）
- QTreeWidget：Project 📁 / Phase 📂 / Task • の3階層
- 右側 DetailPanel：名前・状態・締切・優先度（task）・見積を編集して保存
- ツールバー：＋プロジェクト / ＋フェーズ / ＋タスク / 削除 / ↑↓ / 再読込
- 追加時はダイアログで名前入力 → 作成後に選択状態を維持
- 削除は確認ダイアログあり（cascade）
- 並び替えは order_index の swap（Phase/Task のみ）
- offscreen smoke test + 手動起動確認済み

### 開発を再開するときの読み順

1. このDESIGN.md
2. `pyproject.toml`（依存確認）
3. `src/task_timer/models.py`（データ構造）
4. `src/task_timer/db/repository.py`（使えるCRUD一覧）
5. `src/task_timer/app.py`（現状のエントリポイント）

### コミット状況

ステップ2終了時点で **未コミット**。次回 `git status` で全体を確認してから着手すること。
