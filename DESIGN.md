...
| # | 機能 | 対象ファイル | 概要 |
|---|---|---|---|
| A | **タイマー画面UI刷新** | timer_window.py | フェーズドロップダウンを廃止。プロジェクトとタスクの2段表示に変更。タスク選択肢はフェーズごとに色分け（セクションヘッダーを挿入） |
| B | **タイマー画面でタスク完了** | timer_window.py | タイマー画面にチェックボタン追加。タスクを `done` に更新し、ドロップダウンから除外してカンバン自動リロード |
| C | **締切設定UI** | kanban_window.py | フェーズヘッダー・タスクカードに締切日を表示。クリックでDatePickerダイアログ（プリセット：今日/明日/今週末/来週月 ＋ カレンダー） |
